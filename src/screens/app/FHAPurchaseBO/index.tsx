...
const stateData = [
  {
    ...
    data: [
      ...
      { label: 'Calaveras', value: 2 },
      { label: 'El Dorado', value: 3 },
      { label: 'Los Angeles', value: 4 },
      { label: 'San Diego', value: 5 },
      { label: 'San Francisco', value: 6 },
    ],
  },
  {
    label: 'Florida',
...
const FHAPurchaseBO = (props: IFHAProps) => {
  ...
  let property_price_min = props.defaultData
    ...
    : 0;
  let property_tax = props.defaultData
    ? parseFloat(defaultData.property_tax)
    : 0;
  ...
  const [mortgageTerm, setMortgageTerm] = React.useState('10');
  ...
  const CalculateFHA = () => {
    ...
    request.mortgage_term = mortgageTerm;
    //console.log(request);
    props.fhaCalculation(request);
  };
        ...
        onValueChange={(value: any, label: any) => {
          if (type === 1) {
            ...
            setTimeout(() => {
              ...
              console.log(mortgageTerm, value, label);
            }, 500);
            let myArray = JSON.parse(JSON.stringify(interest_rate));
            let currentData = JSON.parse(JSON.stringify(data));
