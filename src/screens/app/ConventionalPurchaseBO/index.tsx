...
interface IConvetionalProps {
  defaultData: any;
  getDefault: Function;
  conventionalCalculation: Function;
  loading: boolean;
  OpenValidationAlert: Function;
  closeModal: Function;
  conventionalError: string;
...
const ConventionalPurchaseBO = (props: IConvetionalProps) => {
  ...
  React.useEffect(() => {
    if (
      ...
      myCredits.map((item: any, index: number) => {
        tempCredit.push({
          label: item.value + '',
          value: item.id,
        });
    ...
    if (props.conventionalError && !props.loading) {
      let obj = {
        message:
          props.conventionalError !== undefined ? props.conventionalError : '',
        type: 'failure',
      };
      setTimeout(() => {
  ...
  }, [props.defaultData, props.conventionalError]);
  ...
  const CalculateConvetional = () => {
    ...
    //console.log(request);
    props.conventionalCalculation(request);
  };
...
const mapStateToProps = (state: IReduxState) => ({
  ...
  conventionalError: state.calculation.conventionalError,
